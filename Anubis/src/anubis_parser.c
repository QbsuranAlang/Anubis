...
}//end anubis_parse_2_bytes_integer
...
static int anubis_check_config_requirement(anubis_t *config) {
    ...
    if(config->socket_type == anubis_data_link_socket) {
        if(!config->device) {
            ...
            if(!config->device)
                ...
                if(!config->device)
                    ...
                    anubis_verbose("Socket[%d] Data-link: \"Device\" chose \"%s\"\n", config->index, config->device);
            }
            return 0;
        }//end if
    }//end if data-link
        ...
        if(config->role != ANUBIS_ROLE_SERVER && config->role != ANUBIS_ROLE_CLIENT) {
...
static void anubis_parse_socket_config(json_value *json, anubis_t *config) {
    ...
    if(options->type != json_object) {
        anubis_err("%s: \"Option\" should be an object\n", prefix);
        return;
    }//end if
...
#pragma mark others
static void anubis_parse_socket_type(json_value *json, int index) {
    ...
    if(!strcasecmp(socket_type, "Data-link")) {
        ...
        anubis_err("\"%s\" unknown socket type\n", socket_type);
        return;
    }//end else
...
#ifdef WIN32
static bool anubis_init_winsock() {
    WSADATA	WSAData = { 0 };
...
void anubis_parser(const char *filename) {
    ...
    value = json_parse_ex(&settings, (json_char *)file_contents, file_size, errbuf);
    ...
    if(value->type == json_array) {
        ...
        for(int i = 0 ; i < value->u.array.length ; i++) {
            if(asynchronous) {
                pid_t pid = fork();
                if(pid == 0) {
                    anubis_parse_socket_type(value->u.array.values[i], i);
                }//end if
                else if(pid == -1) {
                    anubis_perror("fork()");
                }//end if
